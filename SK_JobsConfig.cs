global using static SK_Jobs.Faction;
...
namespace SK_Jobs
{
    public class Faction
    {
        ...
        public virtual string Format(out IEnumerable<Member> members, out int count)
        {
            members = FindMembers();
            count = members.Count();
            return TranslateJobFormat(Name, count, count == 0 ? 0 : (count * 100d) / Provider.clients.Count, 1);
        }
    ...
    public partial class SK_JobsConfig : IRocketPluginConfiguration
    {
        ...
        public void LoadDefaults()
        {
            Factions.Add(new()
            {
                ...
                Ranks = new()
                {
                    new() { Group = "staff", DisplayName = "Staff" }
                }
            });
        }
    }
